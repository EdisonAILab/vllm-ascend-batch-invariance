"""
vLLM batch invariance final test — validates the fix on Ascend NPU.

Fix: max_num_seqs=1 + attention per-sequence patch ensures bit-exact
batch invariance by processing one sequence at a time through the model.

Usage:
    ASCEND_RT_VISIBLE_DEVICES=0 python3 test_vllm_batch_invariance_final.py [--num_prompts 16] [--max_tokens 32]
"""
import argparse
import json
import os
...
def run_test(prompts, max_tokens, label, **llm_kwargs):
    print(f"\n{'=' * 70}")
    print(f"  {label}")
    print(f"{'=' * 70}")
    sys.stdout.flush()
    ...
    # Singles — separate LLM instance
    print("  Loading LLM for singles...", flush=True)
    ...
    # Batch — separate LLM instance
    print("  Loading LLM for batch...", flush=True)
...
def main():
    ...
    # Test 1: Default (no fix)
    results["Default (no fix)"] = run_test(
        prompts, args.max_tokens,
        "Mode 1: Default (no fix)")

    # Test 2: max_num_seqs=1 (batch invariant mode)
    results["max_num_seqs=1 (batch invariant)"] = run_test(
        prompts, args.max_tokens,
        "Mode 2: max_num_seqs=1 (batch invariant mode)",
        max_num_seqs=1)
